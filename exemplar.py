...
def insert_line(line_id: int, example_id: int, example_step: int, line: str) -> None:
    ...
    cursor.execute('''INSERT INTO example_lines (elid, example_id, step_id, line, line_scheme, line_type) 
...
def process_examples(examples: List) -> None:
    ...
    for line in examples:
        ...
        if previous_line == '':  # Then on first line.
            ...
            assert False, "previous_line must be empty, M, B, or E, not " + previous_line
...
def mark_loop_likely() -> None:
    ...
    CONFIRM: The corresponding terminal conditions are then likewise set (see last cursor.execute() below).
    :return void:
    """
    ...
    # UPDATE example_lines SET loop_likely = 1 WHERE elid IN
    #   (
...
def remove_all_c_labels() -> None:
    ...
    cursor.execute('''SELECT elid, line FROM example_lines WHERE line_type != 'in' ''')  # WHERE loop_likely = 0''')
    #non_looping = cursor.fetchall()
    for row in cursor.fetchall():  #non_looping:
        elid = row[0]
        line = remove_c_labels(row[1])
        query = "UPDATE example_lines SET line = ? WHERE elid = ?"
        cursor.execute(query, (line, elid))
...
def build_reason_evals() -> None:
    ...
    else:  # There are no 'reason's, so select all step 0, loop unlikely, inputs.
        cursor.execute("SELECT line as inp FROM example_lines WHERE step_id = 0 AND loop_likely = 0 AND line_type = 'in'")
    ...
    for inp in cursor.fetchall():
        ...
        cursor.execute("SELECT elid, example_id FROM example_lines WHERE step_id = 0 AND line = ?", (inp,))  # First, fetch inp's elid.
        row_one = cursor.fetchone()
        elid = row_one[0] + 1
        example_id = row_one[1]
        reason = "i1 == " + inp
        cursor.execute("""INSERT INTO example_lines (elid, example_id, step_id, line, line_scheme, line_type) VALUES 
                            (?,?,?,?,?,?)""", (elid, example_id, 1, reason, schematize(reason), 'truth'))
...
def reset_db() -> None:
    ...
    cursor.execute('''CREATE TABLE example_lines (
                        elid INTEGER PRIMARY KEY NOT NULL, -- Sqlite docs recommend not autoincrementing.
    ...
    cursor.execute('''CREATE UNIQUE INDEX e ON example_lines(elid)''')
...
def gen_code() -> str:
    ...
    code += "    return " + get_output(conditions[0][1], conditions[0][2]) + '\n'  # get_output(reason_eid, step_id)
    ...
    for condition in conditions:
        ...
        if condition == "True":  
            ...
            code += "elif " + condition + ":\n"
        code += "    return " + get_output(reason_eid, step_id) + '\n'
...
def get_output(reason_eid: int, step_id: int) -> str:
    ...
    """
    cursor.execute('''SELECT line FROM example_lines WHERE line_type = 'out' AND example_id = ? AND step_id > ? AND 
                        loop_likely = 0 ORDER BY step_id LIMIT 1''', (reason_eid, step_id))
...
def gen_tests(f_name: str) -> str:
    ...
    cursor.execute('''SELECT inp.line, output.line, inp.example_id FROM example_lines inp, example_lines output 
                        ...
                        AND inp.step_id = 0 ORDER BY inp.example_id, output.step_id DESC''')
