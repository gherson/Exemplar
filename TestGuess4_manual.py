...
def get_expected_io(exem: str, example: int) -> str:
    out_exem_lines = []
    example_reached = 0
    for line in exemplar.clean(exemplar.from_file(exem)):
        if not line.strip():
            example_reached += 1
        if (line.startswith('<') or line.startswith('>')) and example == example_reached:
...
class TestGuess4(unittest.TestCase):
    ...
    def test_guess41(self):
        ...
        self.assertEqual(get_expected_io('guess4.exem', example=0), actual_io_trace)

    def test_guess42(self):
        global global_input
        global_input = ['John', '3', '11', '1', '2', '10', '9', '8']  # From an example of the .exem
        guess4()  # The function under test is used to write to actual_io_trace.
        self.assertEqual(get_expected_io('guess4.exem', example=1), actual_io_trace)
